...
def get_minibatch(roidb, num_classes):
    ...
    # Get the input bv images blob (n, channel, h, w), and scales formatted for caffe, gabriel
    im_bv_blob = _get_bvimg_blob(roidb)

    # Now, build the region of interest and label blobs
    rois_blob = np.zeros((0, 5), dtype=np.float32)
    labels_blob = np.zeros((0), dtype=np.float32)
    # gabriel: add rois for bv
    rois_bv_blob = np.zeros((0, 5), dtype=np.float32)
    ...
    for im_i in xrange(num_images):
        labels, overlaps, im_rois, keep_inds, im_rois_bv \
        ...
        # gabriel: bv
        rois_bv_blob_this_image = np.hstack((batch_ind, im_rois_bv))

        # concatenate rois_blob
        rois_blob = np.vstack((rois_blob, rois_blob_this_image))
        rois_bv_blob = np.vstack((rois_bv_blob, rois_bv_blob_this_image)) # gabriel: bv
...
def _sample_rois(roidb, fg_rois_per_image, rois_per_image):
    ...
    rois = rois[keep_inds]
    # gabriel: for bv
    rois_bv = rois_bv[keep_inds]

    return labels, overlaps, rois, keep_inds, rois_bv # add bv
